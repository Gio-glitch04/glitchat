...
client.py con:
 ...
 - Sidebar (salas visitadas y listado de salas públicas)
...
Compatible con el server.py de esta conversación (protocolo de texto simple).
"""
...
class ChatClient:
    ...
    def connect(self):
        ...
        try:
            ...
            s.connect((host, port))
            s.settimeout(2.0)
            # Handshake: servidor envía "NOMBRE"
            prompt = s.recv(1024).decode('utf-8', errors='replace')
            if "NOMBRE" not in prompt.upper():
                messagebox.showwarning("Aviso", "El servidor no envió el prompt esperado.")
            s.sendall((username + "\n").encode('utf-8'))
